...
class Tester:
    def __init__(self, config_parser):
        ...
        self.img = np.zeros(shape=(WIDTH,HEIGHT, 3), dtype=np.uint8)
    ...
    def run(self):
        ...
        while True:
            ...
            table = NetworkTables.getTable('vision/balls')
            table.setDefaultNumber('ry',-1)
            table.setDefaultNumber('by',-1)
            table.setDefaultNumber('rx',-1)
            table.setDefaultNumber('bx',-1)
            ...
            for i in range(len(boxes)):
                if scores[i] > 0.5:
                    ...
                    red = [70, 50, 200]
                    redtolerance = [100, 100, 100]
                    blue = [200, 150, 60]
                    bluetolerance = [100, 100, 100]
                    ...
                    if self.isWithinTolerance(red, averages, redtolerance):
                        ...
                        if Mr < r:
                            Mr = r
                            table.putNumber('rx',(xmin+xmax)/2)
                            table.putNumber('ry',(ymin+ymax)/2)
                            detected_R = True
                        ...
                        if Mb < b:
                            Mb = b
                            table.putNumber('bx',(xmin+xmax)/2)
                            table.putNumber('by',(ymin+ymax)/2)
                            detected_B = True
                    else:
                        class_ids[i] = 2
                        cv2.rectangle(frame_cv2, (xmin, ymin), (xmax, ymax), (0, 255, 0), 2)
            if not detected_R:
                table.putNumber('rx',-1)
                table.putNumber('ry',-1)
            if not detected_B:
                table.putNumber('bx',-1)
                table.putNumber('by',-1)
            for i in range(len(boxes)):
                if scores[i] > .5:
